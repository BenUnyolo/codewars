import { tribonacci } from "./009-tribonacci-sequence";

type caseType = [[number, number, number], number, number[]]
...
describe('testing various routes', () => {
    test.each(cases)( "array %p and n = %p returns %p", (arr, n, result) => {
        expect(tribonacci(arr, n)).toStrictEqual(result);
    });
})